...
RUN /usr/local/go/bin/gofmt -w cmd/kiro-provider/*.go internal/*/*.go \
    ...
    && CGO_ENABLED=1 GOOS=linux GOARCH=amd64 /usr/local/go/bin/go build \
       ...
       -ldflags="-s -w" \
       -o /out/linux/amd64/kiro-provider-v0.7.7.so ./cmd/kiro-provider \
    && rm -f /out/linux/amd64/kiro-provider-v0.7.5.h \
    && cd /out/linux/amd64 \
    && sha256sum kiro-provider-v0.7.7.so > kiro-provider-v0.7.7.so.sha256
