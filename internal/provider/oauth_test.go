...
func TestOrganizationBrowserLoginStartAndPoll(t *testing.T) {
	originalHTTP := hostHTTPDoCall
	originalConfig := loadedConfig()
	t.Cleanup(func() {
		hostHTTPDoCall = originalHTTP
	...
	configValue.Store(pluginConfig{
		ImportMode: "reference", LoginMode: "organization-browser", ModelPrefix: "kiro/",
		SSORegion: "eu-west-1", SSOStartURL: organizationStartURL, ModelDiscoveryURL: "https://service.fixture.invalid",
	})
	...
	hostHTTPDoCall = func(req hostHTTPRequest) (hostHTTPResponse, error) {
		...
		case "https://oidc.eu-west-1.amazonaws.com/client/register":
			...
			if payload["issuerUrl"] != organizationStartURL {
				t.Fatalf("registration payload = %#v", payload)
			}
			redirects, _ := payload["redirectUris"].([]any)
			if len(redirects) != 1 || redirects[0] != defaultIDCRedirectURI {
				t.Fatalf("redirectUris = %#v", redirects)
			}
			return hostHTTPResponse{StatusCode: http.StatusOK, Body: []byte(`{"clientId":"idc-client","clientSecret":"idc-secret","clientSecretExpiresAt":4102444800}`)}, nil
		case "https://oidc.eu-west-1.amazonaws.com/token":
			var payload map[string]string
			if errDecode := json.Unmarshal(req.Body, &payload); errDecode != nil {
				t.Fatal(errDecode)
			}
			if payload["clientId"] != "idc-client" || payload["clientSecret"] != "idc-secret" || payload["code"] != "idc-browser-code" || payload["codeVerifier"] == "" || payload["redirectUri"] != defaultIDCRedirectURI || payload["grantType"] != "authorization_code" {
				t.Fatalf("token payload = %#v", payload)
			}
			return hostHTTPResponse{StatusCode: http.StatusOK, Body: []byte(`{"accessToken":"idc-access","refreshToken":"idc-refresh","expiresIn":3600,"tokenType":"Bearer"}`)}, nil
		case "https://service.fixture.invalid/":
			if req.Headers.Get("Content-Type") != "application/x-amz-json-1.0" || req.Headers.Get("X-Amz-Target") != "AmazonCodeWhispererService.ListAvailableProfiles" {
				t.Fatalf("profile discovery request = %#v", req)
			}
			return hostHTTPResponse{StatusCode: http.StatusOK, Body: []byte(`{"profiles":[{"arn":"arn:aws:codewhisperer:us-east-1:000000000000:profile/idc"}]}`)}, nil
		default:
	...
	authorizeURL, errURL := url.Parse(start.URL)
	if errURL != nil {
		t.Fatal(errURL)
	}
	query := authorizeURL.Query()
	if authorizeURL.Host != "oidc.eu-west-1.amazonaws.com" || authorizeURL.Path != "/authorize" || query.Get("client_id") != "idc-client" || query.Get("state") != start.State || query.Get("redirect_uri") != defaultIDCRedirectURI || query.Get("code_challenge") == "" || query.Get("code_challenge_method") != "S256" {
		t.Fatalf("organization authorize URL = %s", start.URL)
	}

	authDir := t.TempDir()
	callbackPath := filepath.Join(authDir, ".oauth-kiro-"+start.State+".oauth")
	callbackRaw, _ := json.Marshal(oauthCallbackPayload{Code: "idc-browser-code", State: start.State})
	if errWrite := os.WriteFile(callbackPath, callbackRaw, 0o600); errWrite != nil {
		t.Fatal(errWrite)
	}
	pollRequest, _ := json.Marshal(authLoginPollRequest{Provider: providerID, State: start.State, Metadata: start.Metadata, Host: hostConfigSummary{AuthDir: authDir}, HostCallbackID: "callback-poll"})
	...
	if poll.Status != "success" || stored.AuthType != "aws_sso_oidc" || stored.RefreshToken != "idc-refresh" || stored.ClientID != "idc-client" || stored.SSORegion != "eu-west-1" || stored.APIRegion != "us-east-1" || stored.ProfileARN == "" || stored.SourceKind != "oauth_organization_browser" || stored.Label != "Kiro Organization" {
		t.Fatalf("organization poll/stored = %#v / %#v", poll, stored)
	}
	if requests != 3 {
